...
public class SudAudioInputStream extends AudioInputStream {
	...
	/**
	 * The total samples read in all previous buffers. Note that the total samples read
	 * is samplesRead + readIndex. 
	 */
	...
	public SudAudioInputStream(SudFileExpander sudFileExpander, AudioFormat format, long length) {
		...
		sudFileExpander.addSudFileListener((chunkId, chunk)->{
			...
			if (sudFileExpander.getChunkIDString(chunkId).equals("wav")) {
				sudPrint("New wav data: No. bytes: " + chunk.buffer.length + " Total samples read: " +bytesRead + " of " + totalBytes); 
	 ...
	 * <p>
	 * This function  reads a .sud file, figures out if it contains acoustic data, creates an AudioFormat object form header data and
	 * prepares a SudAudioInputStream ready to read audioData. 
	 * @param file - the .sud file.
	 * @return a SudAudioInputStream which can be used to stream audio data e.g. in a similar way to a raw .wav file. 
	 * @throws Exception - throws an exception if the file is incorrect or the .sud file does not contain audio data. 
	 */
	...
	public static SudAudioInputStream openInputStream(File file, boolean verbose) throws Exception
	{
		...
		//Iterate through all the sud chunks to figure out which data handlers the sud files need
			...
			try {
				...
				if (chunkHeader.checkId()) {
					...
					if (chunkHeader.ChunkId==0) {
						...
						//mark the last point at which a ChunkID of 0 is found. Means we don't need to iterate through 
					...
					if (sudFileExpander.getChunkIDString(chunkHeader.ChunkId).equals("wav")) {
						...
						/**
						 * SoundTraps, especially running at high sample rates, might drop samples. The samples are added as zeros (or not added at all). Counting
						 * samples is therefore a little problematic - we must add the zeros if these are indeed implemented. 
						 * 
						 */
						if (lastWavChunk!=null && sudFileExpander.getSudParams().zeroPad) {

							totalSamples = totalSamples+nWavSamples(chunkHeader, lastWavChunk, wavFileHandler.getSampleRate(), sudFileExpander.getSudParams().zeroPad); 

						}
						else {
							//this is the first time a wav chunk has been encountered. Get the sample rate. 
							//Do we have wav file data handlers? If not then this is not an audio stream and throw an exception. 
							ArrayList<IDSudar> dataHandlers =  new ArrayList<IDSudar>(sudFileExpander.getDataHandlers().values());
		...
		SudAudioInputStream sudAudioInputStream =  new SudAudioInputStream(sudFileExpander, audioFormat, totalSamples*(wavFileHandler.getBitsPerSample()/8));
	...
	public static int nWavSamples(ChunkHeader chunkHeader,  ChunkHeader lastWavChunk, float fs, boolean zeroPad) {
		...
		if (lastWavChunk==null) return chunkHeader.SampleCount;
		
		if (!zeroPad) return chunkHeader.SampleCount ;
	...
	private void nextChunk(int bytes2Skip) {
		...
		while(true){
			try {
				//System.out.println("Deserialise: " + sudFileExpander.getSudInputStream().available());
				chunkHeader = ChunkHeader.deSerialise(sudFileExpander.getSudInputStream());
				count++;
				...
				if (chunkHeader.checkId()) {
					...
					if (sudFileExpander.getChunkIDString(chunkHeader.ChunkId).equals("wav")) {
						...
						int bytesInChunk = (this.getFormat().getSampleSizeInBits()/8)*nWavSamples(chunkHeader, lastWavChunk, this.getFormat().getSampleRate(), sudFileExpander.getSudParams().zeroPad); 
						
						if (bytes2SkipLeft<bytesInChunk) {

							sudFileExpander.getSudInputStream().readFully(data);

							sudPrint("Chunk ID: " + chunkHeader.ChunkId + "  magic OK? " + chunkHeader.checkId() + " " + sudFileExpander.getChunkIDString(chunkHeader.ChunkId) + "data len: " + data.length);

							//remember that there are no X3 chunks as such - the wav chunk has a source ID which accesses the chunk...
							//only process chunks if they are XML heades
							sudFileExpander.processChunk(chunkHeader.ChunkId, new Chunk(data, chunkHeader));
							//if we are skipping lots of samples then 
							// the .sud file listener will now save the decompressed wave data to the buffer. This will be read until
						...
						//if not a wav file then process the chunk normally - stuff such as .CSV files should be decompressed and saved to the file system. 
						sudFileExpander.getSudInputStream().readFully(data);
						sudFileExpander.processChunk(chunkHeader.ChunkId, new Chunk(data, chunkHeader));
					}
				}
				...
				//Hmmmmm - this is not the way to do things but there does not seems to be a number of chunks in the header?
	...
	public int read() throws IOException {
		if (audioBuffer==null) throw new IOException("The audio buffer is null");
	...
	public long skip(long n) throws IOException {
		/**
		 * Skipping is a little tricky because we don't want to decompress 
		 * every X3 chunk on the way to getting to the right spot.  
		 */
		
		/**
		 * There are two options here;
		 * 1) There are enough samples in the buffer for a skip
		 * 2) We need to go through chunks to get to the skipped data. 
		 */
	...
	/**
	 * .sud files are a compressed data stream and therefore it is impossible to know the exact number of samples. 
	 */
	@Override
	...
	/**
	 * Mark is not supported in .sud files because we are decompressing data as we go - going back would mean 
	 * finding the position in compressed data which would be very complicated - better to just start again. 
	 */
	@Override
